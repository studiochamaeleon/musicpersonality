...
export async function onRequest(context: PagesFunctionContext) {
  ...
  const title = score === null ? '친구가 음악 궁합을 기다리고 있어요' : `우리 음악 궁합은 ${score}%`;
  const description = score === null
    ? '간단한 음악 취향 검사를 하고 두 사람의 MUSIC 성향을 비교해보세요.'
    : '닮은 취향과 다른 취향, 함께 들으면 좋은 장르를 확인해보세요.';
  ...
  const destination = createAppHash(hostToken, guestToken);
...
<body>
  <p>음악 궁합 결과를 여는 중입니다. <a href="${escapeHtml(destination)}">바로 열기</a></p>
